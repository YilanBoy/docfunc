      ...
      <div class="flex flex-1 justify-between sm:hidden">
        <span>
          @if ($paginator->onFirstPage())
            <span
              class="relative inline-flex cursor-default select-none items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium leading-5 text-gray-500 dark:border-gray-500 dark:bg-gray-700 dark:text-gray-50"
            >
              {!! __('pagination.previous') !!}
            </span>
          @else
            <button
              class="focus:shadow-outline-blue relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out hover:text-gray-500 focus:border-blue-300 focus:outline-none active:bg-gray-100 active:text-gray-700 dark:border-gray-500 dark:bg-gray-800 dark:text-gray-300 dark:hover:text-gray-50"
              type="button"
        ...
        <span>
          ...
          @else
            <span
              class="relative ml-3 inline-flex cursor-default select-none items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium leading-5 text-gray-500 dark:border-gray-500 dark:bg-gray-700 dark:text-gray-50"
            >
              {!! __('pagination.next') !!}
            </span>
          @endif
            ...
            @foreach ($elements as $element)
              ...
              @if (is_string($element))
                <span aria-disabled="true">
                  <span
                    class="relative -ml-px inline-flex cursor-default select-none items-center border border-gray-300 bg-white px-4 py-2 text-sm font-medium leading-5 text-gray-700 dark:border-gray-500 dark:bg-gray-800 dark:text-gray-300"
                  >{{ $element }}</span>
                </span>
              @endif
                  ...
                  <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                    @if ($page == $paginator->currentPage())
                      <span aria-current="page">
                        <span
                          class="relative -ml-px inline-flex cursor-default select-none items-center border border-gray-300 bg-green-100 px-4 py-2 text-sm font-medium leading-5 text-gray-500 dark:border-gray-500 dark:bg-lividus-700 dark:text-gray-300 dark:hover:text-gray-50"
                        >{{ $page }}</span>
                      </span>
                    @else
                      <button
                        class="focus:shadow-outline-blue relative -ml-px inline-flex items-center border border-gray-300 bg-white px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out hover:text-gray-500 focus:z-10 focus:border-blue-300 focus:outline-none active:bg-gray-100 active:text-gray-700 dark:border-gray-500 dark:bg-gray-800 dark:text-gray-300 dark:hover:text-gray-50"
                        type="button"
